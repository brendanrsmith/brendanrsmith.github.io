...
const EMPTY_STATS: Stats = {
  seen: 0,
  processed: { like: 0, repost: 0, post: 0, follow: 0 },
  triggered: 0,
};

function distanceFromSeed(chain: NodeData[]): number {
  if (!chain.length || !chain[0].viaText) return 0;
  if (chain[0].inSeedGraph) return 1;
  let count = 0;
  for (const node of chain) {
    if (!node.viaText) break;
    if (node.inSeedGraph) return count + 1;
    count++;
  }
  return count;
}
...
async function fetchFollows(actor: string): Promise<Record<string, string>> {
  ...
  do {
    const r = await xrpc("app.bsky.graph.getFollows", {
      actor,
      limit: 100,
      cursor,
    });
...
function Quote({ record }: { record: Record<string, unknown> }) {
  const inner = (record.value ?? record.record) as
  ...
  return (
    <div className="post-quote">
      ...
      {typeof inner?.text === "string" && (
        <div className="post-quote-text">{inner.text}</div>
      )}
    </div>
  );
}
...
function Embed({ embed }: { embed: Record<string, unknown> }) {
  ...
  if (t === "app.bsky.embed.images#view") {
    const imgs = embed.images as
      | Array<{ thumb: string; alt?: string }>
      | undefined;
    if (!imgs?.length) return null;
    return (
      <div className="post-images">
        {imgs.map((img, i) => (
          <img key={i} src={img.thumb} alt={img.alt ?? ""} />
        ))}
      </div>
    );
  }
  if (t === "app.bsky.embed.external#view") {
    ...
    return (
      <div className="post-external">
        {ext.thumb && (
          <img className="post-external-thumb" src={ext.thumb} alt="" />
        )}
  ...
  if (t === "app.bsky.embed.recordWithMedia#view") {
    ...
    return (
      <>
        {media && <Embed embed={media} />}
        {rec && <Quote record={rec} />}
      </>
    );
  }
  return null;
...
function PostPanel({
  ...
  const text = (
    post.record as Record<string, string> | undefined
  )?.text?.trim();
    ...
    <div className={cls}>
      <div className="node-info">
        <div className="node-handle">
          <a
            href={`https://bsky.app/profile/${encodeURIComponent(node.actor)}`}
            target="_blank"
            rel="noopener"
          >
            @{node.actor}
          ...
          <div className="node-via">
            ...
            {node.likedUri && (
              <>
                {" "}
                ·{" "}
                <a href={postUrl(node.likedUri)} target="_blank" rel="noopener">
                  ↗
                </a>
              </>
            )}
...
export default function BskyChain() {
  ...
  const statsRef = useRef<Stats>({
    ...EMPTY_STATS,
    processed: { ...EMPTY_STATS.processed },
  });
  const debounce = useRef<ReturnType<typeof setTimeout> | null>(null);
  const statsInterval = useRef<ReturnType<typeof setInterval> | null>(null);
  const postExitTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    const close = (e: MouseEvent) => {
      if (!(e.target as Element).closest(".input-wrap")) setSuggestions([]);
    };
  ...
  useEffect(() => {
    const snap = () =>
      setStatsSnap({
        ...statsRef.current,
        processed: { ...statsRef.current.processed },
      });
    if (statsOpen) {
      snap();
      statsInterval.current = setInterval(snap, 500);
    } else {
  ...
  function schedulePostUpdate(
    ...
    setPostExiting(true);
    postExitTimer.current = setTimeout(() => {
      setPostExiting(false);
      setCurrentPost(pendingPost.current);
      setPostRevision((r) => r + 1);
    }, 220);
  }
  ...
  async function fetchSuggestions(q: string) {
    try {
      const { actors } = await xrpc("app.bsky.actor.searchActorsTypeahead", {
        q,
        limit: 6,
      });
      setSuggestions(
        actors.length ? actors.map((a: { handle: string }) => a.handle) : [],
      );
      setActiveIdx(-1);
    } catch {
      setSuggestions([]);
    ...
    socket.addEventListener("message", async (e) => {
      ...
      try {
        ...
        if (col === "app.bsky.feed.like") statsRef.current.processed.like++;
          ...
          statsRef.current.processed.repost++;
        else if (col === "app.bsky.feed.post")
          statsRef.current.processed.post++;
        else if (col === "app.bsky.graph.follow")
          statsRef.current.processed.follow++;
        ...
        if (hopping.current) {
          ...
          if (col === "app.bsky.feed.like") {
            ...
            verb = "posted";
            shadowUrl = postUrl(
              `at://${evt.did}/app.bsky.feed.post/${evt.commit.rkey}`,
            );
          } else if (col === "app.bsky.graph.follow") {
            const did = evt.commit.record?.subject;
            if (did) {
              verb = "followed";
              shadowUrl = `https://bsky.app/profile/${did}`;
            }
          }
          if (verb && shadowUrl) {
            setChain((prev) => {
              ...
              return [
                {
                  ...
                  branches: [
                    ...first.branches,
                    { verb: verb!, actorHandle, url: shadowUrl! },
                  ],
                },
                ...rest,
              ];
        ...
        const [profileRes, postsRes] = await Promise.allSettled([
          ...
          targetUri
            ? xrpc("app.bsky.feed.getPosts", { uris: targetUri })
        ...
        if (followsCount < MIN_FOLLOWS) {
          setSkips((s) => s + 1);
          setStatus(
            `skipping @${profile.handle} (${followsCount} follows) — waiting…`,
          );
          hopping.current = false;
          return;
        }
        ...
        const nextPost =
          targetUri && postsRes.status === "fulfilled"
            ? (postsRes.value?.posts?.[0] ?? null)
            : null;
        const viaText =
  ...
  async function reconnect() {
    ...
    connectJetstream(
      ...
      Object.keys(profiles.current).length,
    );
  }
  ...
  // --- stats (derived + snapshotted) ---

  const f = (n: number) => (n === 0 ? "—" : n.toLocaleString());
  const hops = chain.filter((n) => n.viaText !== null).length;
  const branches = chain.reduce((sum, n) => sum + n.branches.length, 0);
  const fromSeed = distanceFromSeed(chain);
  const matched =
    statsSnap.processed.like +
    statsSnap.processed.repost +
    statsSnap.processed.post +
    statsSnap.processed.follow;
      ...
      <div className="layout">
              ...
              {statsOpen && (
                <div className="info-popover stats-popover open">
                  <div className="stats-section">
                    ...
                    <div className="stats-row">
                      <span className="sk">skips</span>
                      <span className="sv">{f(skips)}</span>
                    </div>
                  </div>
                  ...
                  <div className="stats-section">
                    ...
                    <div className="stats-row">
                      <span className="sk">likes</span>
                      <span className="sv">{f(statsSnap.processed.like)}</span>
                    </div>
                    <div className="stats-row">
                      <span className="sk">reposts</span>
                      <span className="sv">
                        {f(statsSnap.processed.repost)}
                      </span>
                    </div>
                    <div className="stats-row">
                      <span className="sk">posts</span>
                      <span className="sv">{f(statsSnap.processed.post)}</span>
                    </div>
                    <div className="stats-row">
                      <span className="sk">follows</span>
                      <span className="sv">
                        {f(statsSnap.processed.follow)}
                      </span>
                    </div>
                  </div>
                  <div className="stats-divider" />
                  <div className="stats-section">
                    ...
                    <div className="stats-row">
                      <span className="sk">triggered</span>
                      <span className="sv">{f(statsSnap.triggered)}</span>
                    </div>
                    <div className="stats-row">
                      <span className="sk">branches</span>
                  ...
                  <div className="stats-section">
                    ...
                    <div className="stats-row">
                      <span className="sk">seen</span>
                      <span className="sv">{f(statsSnap.seen)}</span>
                    </div>
                    <div className="stats-row">
                      <span className="sk">matched</span>
                      <span className="sv">{f(matched)}</span>
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="post-col">
          <div id="post-panel">
            <div
              key={postRevision}
              className={`post-panel-inner${postExiting ? " exiting" : ""}`}
            >
              <PostPanel
